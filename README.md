...
![The landing page: the pitch beside the merchant's live policy](docs/screenshots/01-hero.png)

<sub>The panel on the right is not a mockup — it is the merchant's real policy, read from
Postgres on load: floor margin, daily discount budget, repeat-buyer cooldown, and the
funding order with what each step costs.</sub>
...
![Protocol surfaces: the agentic feed and the ed25519 key set](docs/screenshots/08-protocol.png)

<sub>Two live endpoints, fetched in the browser at `/protocol`. `/acp/feed` is what a buying
agent reads before it ever opens a session; `/.well-known/jwks.json` is the key set a
counterparty uses to verify a signed counter-offer without trusting us.</sub>
...
![The live control room: 13 buyers against one shared budget](docs/screenshots/03-live-run.png)

<sub>Buyers 1–7 walk every funding source and both attachment outcomes; 8–13 put the same
engine under adverse conditions. They run one at a time, because the campaign budget they
share depletes as they spend it. Each buyer holds at its outcome until you advance, or you
hand the rest to the clock.</sub>
...
![The four funding sources, side by side](docs/screenshots/02-waterfall.png)

<sub>Shown as parallel columns rather than a stepper, because the order itself is the
argument. Only the fourth spends merchant margin — and it is the only one the floor can
refuse outright.</sub>
...
Budgets are finite and shared, which is what makes the order matter in practice:

![The campaign runs dry and the waterfall steps down](docs/screenshots/04-fallthrough.png)

<sub>The behaviour the whole design turns on. Ananya and Rohit both take the Nike campaign,
draining it to ₹0. Meera then arrives with an **identical instruction and an identical
cart** — and the waterfall falls through: *“Brand campaign — skipped. Nike Summer Sale
would have covered this, but the budget is spent.”* She still closes, funded by a bank
rail offer, still at ₹0 merchant cost. Note the plain-English receipt and the ed25519
ledger-tip signature beneath the outcome.</sub>
...
![Transaction history with a session expanded](docs/screenshots/07-history.png)

<sub>Read back from the `sessions` table, so it survives a restart. Settlements and refusals
sit side by side, and any row expands into its mandate, cart hash at consent, waterfall
trace and settlement. A refused offer is marked *(offered)* and *not spent* — an offer the
buyer declined never counts against merchant margin.</sub>
...
Every figure below is generated, not asserted — regenerate the whole set with `npm run metrics`.

![Analytics: revenue and gross profit per buyer](docs/screenshots/05-analytics.png)

<sub>The run hands off to a full analytics view. Revenue by buyer is coloured by the funding
source that closed it; gross profit is priced against **real unit cost** from the catalog
rather than modelled; refusals are shown as refusals rather than as gaps.</sub>
...
The live run reports the same comparison on whatever cohort just executed:

![The counterfactual: the same cohort under three policies](docs/screenshots/06-counterfactual.png)

<sub>The same buyers, replayed under no-rescue and under a blanket 10% off. The blanket
discount frequently earns *less than doing nothing*, because it converts more but gives away
margin on carts that would have closed anyway. The card beside it isolates the sessions where
the merchant actually paid. (The counterfactual compares list price against each buyer’s hard
cap and ignores the stretch rule, which flatters the blanket arm — the real gap is wider than
shown.)</sub>
